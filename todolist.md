...
- [ ] **SS kernel L2** (currently L1+L3 only) — **UNBLOCKED 2026-08-12, no VM needed.** The blocker was CTF2, not the VM: SS metadata is CTF 2 (`{"type":"preamble","version":2,…}`) and needs babeltrace ≥2.1. Built **babeltrace 2.1.2** at `/scratch/yuvraj17/local-bt21` and verified it decodes a real SS trace (2.0.4 fails on the same one). Run it with `LD_LIBRARY_PATH=/scratch/yuvraj17/local-bt21/lib:$LD_LIBRARY_PATH` (the old 2.0.4 at `/scratch/yuvraj17/local/lib` shadows it). Remaining work = a per-run batch job: extract L0 from the `/project` archive (~11 GB decompressed/run) → derive L2 → keep the tiny jsonl → discard L0. Also re-derive the missing **L1/L3** for SS `normal` ×6, `anomaly_mem` calib ×4, `lttng_only` ×4. **GCP VMs are unreachable (billing disabled) — do not plan around them.**
