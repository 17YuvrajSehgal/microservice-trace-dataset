...
## The results in four charts

Generated by `blueprints/lib/q2_charts.py` from the same run files as the tables below.

### 1. Where the blueprint helps, and where it hurts

![Blueprint effect by problem and measure](charts/1-blueprint-effect.png)

Read the rows in three groups. Faults that hit the **whole host** are at the top, a **datastore** fault in the middle, **one service** at the bottom.

Three things stand out. The blueprint helps most at explaining the problem - +44 on `noisy_neighbor`, +29 on `slow_db`. It does nothing for finding the place on three problems, and the two reasons for that are opposite (chart 3). And on `anomaly_net` it makes finding the time **37 points worse** - that one is our own fault and is explained at the end of this file.

### 2. How good is the agent, with and without?

![Accuracy with and without a blueprint](charts/2-accuracy.png)

The heatmap shows the change. This shows the level, which matters just as much: a +23 from 67%% to 90%% is not the same as a +23 from 0%% to 23%%.

Finding the place splits hard by scope. Host-wide faults run 90-100%% with a blueprint. The two single-service faults sit at **zero in both arms**.

### 3. Why the blueprint changes nothing on three problems

![Score without vs with a blueprint](charts/3-room-to-improve.png)
...
`anomaly_cpu` sits top right at 100%% in both arms. The agent already found it every time, so there was nothing left to gain.

`svc_cpu_cap` and `svc_net` sit bottom left at zero in both arms. The agent could not find it either way, because our tools could not tell one container's `java` from another's. A blueprint cannot rescue a question the evidence cannot answer.

**Same number, opposite meaning.** That is why we do not report an average across problems - it came out as +0 and told us nothing.

### 4. What it costs

![Time and tokens with and without a blueprint](charts/4-cost.png)

The blueprint adds roughly 10-25k tokens per run, because it is a long document to read. It does **not** cost time. On five of six problems the agent is faster with it - it stops casting about and follows a method.
