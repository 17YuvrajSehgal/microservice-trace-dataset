#!/usr/bin/env python3
"""Does the packet-loss signature hold across every run and both applications?
...
FIELDS = [("n_impaired", "interfaces impaired"),
          ("worst_retrans_pct", "worst retransmit %"),
          ("median_retrans_pct_baseline", "baseline retransmit %"),
          ("worst_drop_pct", "worst drop %")]
NETWORK_FAMILIES = {"anomaly_net", "svc_net"}
...
def main():
    ...
    ap.add_argument("--out", default="netloss_summary.json")
    a = ap.parse_args()
    ...
    for r in sorted(rows, key=lambda r: (r["app"], r["family"], r["run_id"])):
        ...
        print(f"{r['run_id'][:42]:42s} {r['family']:18s} "
              f"{str(r.get('n_impaired')) + '/' + str(r.get('n_interfaces')):>10s} "
              f"{f(r.get('worst_retrans_pct'), 8)} {f(r.get('median_retrans_pct_baseline'))} "
              f"{f(r.get('worst_drop_pct'))}")
    ...
    for app in sorted({r["app"] for r in rows}):
        ...
        net = [r for r in ar if r["is_network"] and isinstance(r.get("worst_retrans_pct"), float)]
        other = [r for r in ar
                 if not r["is_network"] and isinstance(r.get("worst_retrans_pct"), float)]
        if not (net and other):
            continue
        nlo = min(r["worst_retrans_pct"] for r in net)
        ohi = max(r["worst_retrans_pct"] for r in other)
        worst_other = max(other, key=lambda r: r["worst_retrans_pct"])
        verdict = "CLEAN SPLIT" if nlo > ohi else "OVERLAP"
        print(f"  {app:12s} network worst-retransmit floor {nlo:.2f}%  "
              f"non-network ceiling {ohi:.2f}% ({worst_other['family']})  -> {verdict}")
        nimp = [r["n_impaired"] for r in net if isinstance(r.get("n_impaired"), int)]
        oimp = [r["n_impaired"] for r in other if isinstance(r.get("n_impaired"), int)]
        if nimp and oimp:
            print(f"  {'':12s} interfaces impaired: network {min(nimp)}-{max(nimp)}, "
                  f"non-network {min(oimp)}-{max(oimp)}")
