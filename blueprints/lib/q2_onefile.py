...
def main() -> int:
    ...
    L.append("The middle value of the 15 runs, not the average. One very slow run cannot drag "
             "the number.")
    L.append("")
    L.append("## The results in four charts")
    L.append("")
    ...
    L.append("")
    L.append("Read the rows in three groups. Faults that hit the **whole host** are at the "
             "top, a **datastore** fault in the middle, **one service** at the bottom.")
    L.append("")
    L.append("Three things stand out. The blueprint helps most at explaining the problem - "
             "+44 on `noisy_neighbor`, +29 on `slow_db`. It does nothing for finding the place "
             "on three problems, and the two reasons for that are opposite (chart 3). And on "
             "`anomaly_net` it makes finding the time **37 points worse** - that one is our "
             "own fault and is explained at the end of this file.")
    L.append("")
    L.append("### 2. How good is the agent, with and without?")
    L.append("")
    L.append("![Accuracy with and without a blueprint](charts/2-accuracy.png)")
    L.append("")
    L.append("The heatmap shows the change. This shows the level, which matters just as much: "
             "a +23 from 67%% to 90%% is not the same as a +23 from 0%% to 23%%.")
    L.append("")
    L.append("Finding the place splits hard by scope. Host-wide faults run 90-100%% with a "
             "blueprint. The two single-service faults sit at **zero in both arms**.")
    L.append("")
    L.append("### 3. Why the blueprint changes nothing on three problems")
    L.append("")
    L.append("![Score without vs with a blueprint](charts/3-room-to-improve.png)")
    L.append("")
    ...
    L.append("")
    L.append("`anomaly_cpu` sits top right at 100%% in both arms. The agent already found it "
             "every time, so there was nothing left to gain.")
    L.append("")
    L.append("`svc_cpu_cap` and `svc_net` sit bottom left at zero in both arms. The agent could "
             "not find it either way, because our tools could not tell one container's `java` "
             "from another's. A blueprint cannot rescue a question the evidence cannot answer.")
    L.append("")
    L.append("**Same number, opposite meaning.** That is why we do not report an average across "
             "problems - it came out as +0 and told us nothing.")
    L.append("")
    L.append("### 4. What it costs")
    L.append("")
    L.append("![Time and tokens with and without a blueprint](charts/4-cost.png)")
    L.append("")
    L.append("The blueprint adds roughly 10-25k tokens per run, because it is a long document "
             "to read. It does **not** cost time. On five of six problems the agent is faster "
             "with it - it stops casting about and follows a method.")
    L.append("")
    L.append("---")
    L.append("")
