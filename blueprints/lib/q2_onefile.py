...
def main() -> int:
    ...
    ap.add_argument("--charts", default="charts", help="relative path the markdown uses")
    # anomaly_net in every matrix before 22-09 late evening was run against a FALSE kernel
    # recipe that told the agent its deciding check was impossible. Those cells measure our
    # mistake, not the blueprint, so the corrected re-run is substituted here rather than
    # quietly reported. See docs/WHEN-A-BLUEPRINT-HURTS.md.
    ap.add_argument("--override", default="", help="problem=dir, repeatable, comma-separated")
    ap.add_argument("--out", required=True)
    a = ap.parse_args()

    full, ns = load(a.full), load(a.ns) if a.ns else []
    swapped = []
    for spec in [x for x in a.override.split(",") if x.strip()]:
        prob, d = spec.split("=", 1)
        repl = [r for r in load(d) if r.get("problem") == prob]
        if repl:
            full = [r for r in full if r.get("problem") != prob] + repl
            swapped.append((prob, d, len(repl)))
    problems = list(Q.PROBLEMS)
    L = []
    L.append("# %s results - 22 September 2026" % a.app_name)
    L.append("")
    ...
    L.append("| application | **%s** |" % a.app_name)
    L.append("| results | `%s` |" % a.full)
    for prob, d, n in swapped:
        L.append("| `%s` | **taken from `%s`** (%d runs) - the matrix cells for "
                 "it were run against a false kernel recipe, see "
                 "WHEN-A-BLUEPRINT-HURTS.md |" % (prob, d, n))
    L.append("")
    ...
    L.append("")
    L.append("![Blueprint effect by problem and measure](%s/" % a.charts + "1-blueprint-effect.png)")
    L.append("")
    ...
    L.append("")
    L.append("![All 360 runs, one square each](%s/" % a.charts + "2-every-run.png)")
    L.append("")
    ...
    L.append("")
    L.append("![All 360 window scores](%s/" % a.charts + "3-window-raw.png)")
    L.append("")
    ...
    L.append("")
    L.append("![Score without vs with a blueprint](%s/" % a.charts + "4-room-to-improve.png)")
    L.append("")
    ...
    L.append("")
    L.append("![Time and tokens for every run](%s/" % a.charts + "5-cost-raw.png)")
    L.append("")
